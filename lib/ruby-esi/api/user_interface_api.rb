...
module ESI
  class UserInterfaceApi
    ...
    # @option opts [String] :token Access token to use if unable to set a header
    # @return [nil]
    def post_ui_openwindow_newmail(body, opts = {})
      post_ui_openwindow_newmail_with_http_info(body, opts)
      nil
    end
    ...
    # @option opts [String] :token Access token to use if unable to set a header
    # @return [Array<(nil, Integer, Hash)>] nil, response status code and response headers
    def post_ui_openwindow_newmail_with_http_info(body, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: UserInterfaceApi.post_ui_openwindow_newmail ...'
      end
      # verify the required parameter 'body' is set
      if @api_client.config.client_side_validation && body.nil?
        fail ArgumentError, "Missing the required parameter 'body' when calling UserInterfaceApi.post_ui_openwindow_newmail"
      end
      if @api_client.config.client_side_validation && opts[:'datasource'] && !['tranquility'].include?(opts[:'datasource'])
        fail ArgumentError, 'invalid value for "datasource", must be one of tranquility'
      end
