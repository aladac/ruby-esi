...
module ESI
  class IncursionsApi
    ...
    def get_incursions(x_compatibility_date, opts = {})
      data, _status_code, _headers = get_incursions_with_http_info(x_compatibility_date, opts)
      data
    end
    ...
    # @option opts [String] :x_tenant The tenant ID for the request. (default to 'tranquility')
    # @option opts [String] :if_modified_since The date the resource was last modified. A 304 will be returned if the resource has not been modified since this date.
    # @return [Array<(Array<IncursionsGetInner>, Integer, Hash)>] Array<IncursionsGetInner> data, response status code and response headers
    def get_incursions_with_http_info(x_compatibility_date, opts = {})
      ...
      if @api_client.config.client_side_validation && x_compatibility_date.nil?
        fail ArgumentError, "Missing the required parameter 'x_compatibility_date' when calling IncursionsApi.get_incursions"
      end
      # verify enum value
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'Array<IncursionsGetInner>'
